			...
			if(vOLDLength[stepID] > 0) {
				switch(vOLDDir[stepID]) {
					...
					case eBOTRIGHTBOSS: // BOT & RIGHT
						...
						break;
					case eBOSSTEXT1:
						CCore::getMap()->addText(vOLDLength[stepID], CCFG::GAME_HEIGHT - 16 - 9*32, "THANK YOU MARIO.");
						vOLDLength[stepID] = 0;
						break;
					case eBOSSTEXT2:
						CCore::getMap()->addText(vOLDLength[stepID] + 16, CCFG::GAME_HEIGHT - 16 - 7*32, "BUT OUR PRINCESS IS IN.");
						CCore::getMap()->addText(vOLDLength[stepID] + 16, CCFG::GAME_HEIGHT - 16 - 6*32, "ANOTHER CASTLE.");
						vOLDLength[stepID] = 0;
						break;
